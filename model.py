...
class Model(object):
    def __init__(self,n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling = False):
        with tf.name_scope("Inputs"):
            self.mid_his_batch_ph = tf.placeholder(tf.int32,[None,None],name='mid_his_batch_ph') # 用户历史行为的item id list  [B,T]
            self.cat_his_batch_ph = tf.placeholder(tf.int32,[None,None],name='cat_his_batch_ph') # 用户历史行为item对应的cate id list
            self.uid_batch_ph = tf.placeholder(tf.int32,[None,],name='uid_batch_ph') # 用户id[B]
        ...
        with tf.name_scope("Embedding_layer"):
            ...
            self.uid_batch_embedded = tf.nn.embedding_lookup(self.uid_embeddings_var,self.uid_batch_ph) # 查询出当前用户的embedding

            self.mid_embeddings_var = tf.get_variable("mid_embedding_var",[n_mid,EMBEDDING_DIM]) # 构建item embedding
            tf.summary.histogram('mid_embeddings_var',self.mid_embeddings_var)
            self.mid_batch_embedded = tf.nn.embedding_lookup(self.mid_embeddings_var,self.mid_batch_ph) # 查询出 target的item embedding


            self.mid_his_batch_embedded = tf.nn.embedding_lookup(self.mid_embeddings_var,self.mid_his_batch_ph)   # 查询用户历史行为的item embedding
            if self.use_negsampling:
                self.noclk_mid_his_batch_embedded = tf.nn.embedding_lookup(self.mid_embeddings_var,
                                                                           self.noclk_mid_batch_ph)  # 负采样，没有点击的item embedding

            self.cat_embeddings_var = tf.get_variable("cat_embedding_var", [n_cat, EMBEDDING_DIM]) # 构建种类的 embedding集合
            tf.summary.histogram('cat_embeddings_var', self.cat_embeddings_var)
            self.cat_batch_embedded = tf.nn.embedding_lookup(self.cat_embeddings_var, self.cat_batch_ph) #  查询target item 的种类 embedding
            self.cat_his_batch_embedded = tf.nn.embedding_lookup(self.cat_embeddings_var, self.cat_his_batch_ph) # 查询用户历史行为的 item 的种类的embedding
            if self.use_negsampling:
                self.noclk_cat_his_batch_embedded = tf.nn.embedding_lookup(self.cat_embeddings_var,
                                                                           self.noclk_cat_batch_ph)    # 负采样


        self.item_eb = tf.concat([self.mid_batch_embedded,self.cat_batch_embedded],1) # target item 即target cate 的embedding拼接 [B,H/2]+[B,H/2] = [B,H]
        self.item_his_eb = tf.concat([self.mid_his_batch_embedded,self.cat_his_batch_embedded],2) # 历史行为 item 和 历史行为 cate 拼接 [B,T,H/2] ->[B,T,H]
    ...
    def auxiliary_net(self,input,stag='auxiliary_net'):
        ...
        # 这里最后是一个二分类 点击or not 经过softmax 后 变成[点击的概率，未点击的概率]
...
class Model_DIN_V2_Gru_Vec_attGru_Neg(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        # 计算辅助loss，用下一时刻真实的行为去监督当前时刻的隐藏层，即rnn_outputs. 未发生的行为作为负样本
